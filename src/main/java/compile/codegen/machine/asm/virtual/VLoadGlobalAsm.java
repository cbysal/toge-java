...
public record VLoadGlobalAsm(Reg dest, String src) implements Asm {
    @Override
    ...
    public Asm replaceVRegs(Map<VReg, MReg> vRegToMReg) {
        if (dest instanceof VReg vDest && vRegToMReg.containsKey(vDest)) {
            return new VLoadGlobalAsm(vRegToMReg.get(vDest), src);
        }
        return this;
    }
    ...
    public List<Asm> spill(Map<VReg, Integer> vRegToSpill) {
        if (dest instanceof VReg && vRegToSpill.containsKey(dest)) {
            ...
            return List.of(new VLoadGlobalAsm(newDest, src), new StoreAsm(newDest, MReg.SP, spill1, 8));
        }
        return List.of(this);
    }
}
