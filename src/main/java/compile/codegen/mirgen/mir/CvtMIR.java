...
public class CvtMIR implements MIR {
    private Reg target, source;

    public CvtMIR(Reg target, Reg source) {
        this.target = target;
        this.source = source;
    }
    ...
    public String toString() {
        return switch (target.getType()) {
            case FLOAT -> {
                if (source.getType() != Type.INT)
                    throw new RuntimeException();
                yield String.format("fcvt.s.w\t%s,%s", target, source);
            }
            case INT -> {
                if (source.getType() != Type.FLOAT)
                    throw new RuntimeException();
                yield String.format("fcvt.w.s\t%s,%s,rtz", target, source);
            }
